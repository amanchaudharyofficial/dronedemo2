...
import reciever 
from sender import send_message, sendMsg, sendInit,send_udp_tts_message
...
def send_thread():
  ...
  #DEST = "192.168.110.102"
  #DEST = "192.168.111.46"  # IP Arohan
  DEST = "192.168.171.201"  # Change to target IP if needed
  PORT = 5100
  # sendMsg(DEST,PORT,)
def audio_thread():
  ...
  recognizer_english = vosk.KaldiRecognizer(vosk_model_english, 18000)
  recognizer_hindi = vosk.KaldiRecognizer(vosk_model_hindi, 18000)
  #recognizer = sr.Recognizer()
    # Initialize PyAudio
  p = pyaudio.PyAudio()
  stream = p.open(format=pyaudio.paInt16, channels=1, rate=10000, input=True, frames_per_buffer=12000, start=True)
  ...
  try:
    while True:
        ...
        if recognizer_english.AcceptWaveform(data) or recognizer_hindi.AcceptWaveform(data):
            ...
            #send_udp_tts_message(text_hindi)
            # text_hindi=""
            if(text_english==""):
               print("Speak now")
            ...
            # if ("charlie" in text) or ("charly" in text) or ("char" in text) or ("charley" in text):
              if "lost" in text_english:
                  ...
                  sendMsg(DEST,PORT,text_english)
                  send_udp_tts_message(text_english)
            ...
            if(text_hindi==""):
               print("Speak now")
            if ("सत्यवान" in text_hindi) or ( "शक्तिमान" in text_hindi) or ("शक्ति" in text_hindi) or ("परिचय" in text_hindi):
            # if ("charlie" in text) or ("charly" in text) or ("char" in text) or ("charley" in text):
              if "परिचय" in text_hindi:
                  text_hindi="अपना परिचय दें"
                  print("HINDI MESSAGE SENT अपना परिचय दें")
                  sendMsg(DEST,PORT,text_hindi)
                  send_udp_tts_message(text_hindi)
              ...
              if "जानकारी" in text_hindi or "कि संदिग्ध इमारत" in text_hindi:
                  text_hindi="हमें जानकारी मिली है कि संदिग्ध इमारत में छिपा हुआ है"
                  sendMsg(DEST,PORT,text_hindi)
                  send_udp_tts_message(text_hindi)
                  print("HINDI MESSAGE SENT हमें जानकारी मिली है कि संदिग्ध इमारत में छिपा हुआ है")

                  #संदिग्ध को ढूंढ़ने और उसे निष्क्रिय करने के लिए एक मिशन की योजना बनाएं
              if "संदिग्ध को" in text_hindi or "निष्क्रिय करने " in text_hindi:
                  text_hindi="संदिग्ध को ढूंढ़ने और उसे निष्क्रिय करने के लिए योजना बनाएं"
                  sendMsg(DEST,PORT,text_hindi)
                  send_udp_tts_message(text_hindi)
                  print("HINDI MESSAGE SENT संदिग्ध को ढूंढ़ने और उसे निष्क्रिय करने के लिए योजना बनाएं")
              ...
              if "पहचान" in text_hindi or "सही है" in text_hindi:
                  text_hindi="पहचान सही है"
                  sendMsg(DEST,PORT,text_hindi)
                  send_udp_tts_message(text_hindi)
                  print("HINDI MESSAGE SENT पहचान सही है")
              ...
              if "हाँ यह संदिग्ध इमारत है" in text_hindi:
                  text_hindi="हाँ यह संदिग्ध इमारत है "
                  sendMsg(DEST,PORT,text_hindi)
                  send_udp_tts_message(text_hindi)
                  print("HINDI MESSAGE SENT हाँ यह संदिग्ध इमारत है ")
