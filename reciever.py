...
#HOST ="10.10.40.18"
HOST ="192.168.171.67"  # Listen on all interfaces
#HOST ="192.168.110.131"
PORT = 5004
...
# p.terminate()
     ...
     asr_data=STRUCT_MFD_ASR_START_STOP_VOICE_COMMAND()
     memset(addressof(asr_data), 0, sizeof(asr_data))
     memmove(addressof(asr_data), buffer, sizeof(asr_data))

    #  Flag=asr_data.start_stop_command.start_stop_flag
    #  print("flag = ",Flag)

     print("src dest ",msg_header.src_csci_id, msg_header.dest_csci_id)
     msg_id_rcvd=temp_buffer[2]
     Flag=temp_buffer[7]
    #  msg_id_rcvd = msg_header.msg_id
     dest_id_rcvd = temp_buffer[1]
     
     print ("msg id: ",msg_id_rcvd ,"Flag: ",Flag)
    #  if(dest_id_rcvd == CSCI_ID_ENUM.NSA.value):
     if(dest_id_rcvd == 10):
         if (msg_id_rcvd == 20 and Flag==1):
            main.flag=1
            main.streamFlag=1
           
            # audio_thread = Thread(target=main.audio_thread_working)
            # main.audio_thread_working()
            
            
            # IndianEnglishModel.speech_to_text()

            print("msg identified: MFD_ASR_START_STOP_COMMAND_MSG_ID")
            ...
            # elif (msg_id_rcvd == MFD_ASR_START_STOP_COMMAND_MSG_ID and flag == 0):
                # Stop listening and clean up
            
             
         
         
         elif (msg_id_rcvd == 20 and Flag==0):
             main.flag=0 
             main.streamFlag=1
             IndianEnglishModel.speech_to_text() 
             print("Flag set to 0")
